...
**Smart MCP server** already deployed on your Puppet Enterprise
...
This image is thin-client-only. It does not deploy a Smart MCP server —
...
# what to do if you don't have a Smart MCP server yet
...
`setup` first asks whether you already have a Smart MCP server deployed:
...
- **Yes** — it asks for the Smart MCP's URL and its PE CA certificate
...
|---|---|---|
| `setup` | Yes | One-time wizard: deploy-first coaching, or collect Smart MCP URL + CA cert and persist them to `/config` |
| `validate` | No (but human-readable) | Self-check: real `tools/list` handshake against the configured Smart MCP; reports PASS with discovered tools, or a clear FAIL diagnostic |
...
All three read config from `/config/config.env` + `/config/pe-ca.pem` (written by `setup`), or you can skip the mounted volume entirely and pass `-e SMART_MCP_URL=... -e PE_CA_CERT=/path/to/mounted/cert.pem` directly on `docker run`.

## Troubleshooting

- **"SMART_MCP_URL is not configured"** — you haven't run `setup` yet, or the mounted `/config` volume doesn't match what you used during setup. Re-run `setup`.
- **"could not reach or authenticate to the Smart MCP"** (from `validate`) — check the Smart MCP URL is correct and reachable, and that the CA cert matches the certificate your Smart MCP's nginx actually presents (it's signed by your PE CA, not a public one — see below).
- **Don't have a Smart MCP server yet?** See [`puppetlabs-pe_mcp`](https://github.com/puppetlabs/puppetlabs-pe_mcp#quickstart) — deploy one with `bolt plan run pe_mcp::deploy`, then come back and run `setup` here.
